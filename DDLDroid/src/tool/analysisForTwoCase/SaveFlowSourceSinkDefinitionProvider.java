...
public class SaveFlowSourceSinkDefinitionProvider implements ISourceSinkDefinitionProvider {
    ...
    public static SaveFlowSourceSinkDefinitionProvider newSaveFlow(String sinkFileName) throws IOException {
        SaveFlowSourceSinkDefinitionProvider sourcesAndSinks = new SaveFlowSourceSinkDefinitionProvider();
        sourcesAndSinks.readFile(sinkFileName); // 这一步仅载入存在txt中的Sink点
        sourcesAndSinks.readTempFile("./temp.txt"); // 加载ViewModel中的
        return sourcesAndSinks;
    }

    public SaveFlowSourceSinkDefinitionProvider() {
    }
    ...
    /**
     * 向保存数据流检测中添加自定义的 statement-based Source*/
